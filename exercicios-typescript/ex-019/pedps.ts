const evenDigitSquares = (a: number, b: number): number[] => {

  function allEven(n: number):boolean {
  let digs = n.toString().split('');
  let oddDigs = ["1","3","5","7","9"];
  for (let i in oddDigs) {
      let dS = oddDigs[i];
      if (digs.indexOf(dS) != -1) return false;
  }
  return true;
}

function evenDigitSquaresMine(a: number, b: number) {
  let lim1  = Math.floor(Math.sqrt(a)), lim2 = Math.ceil(Math.sqrt(b));
  let res = [];
  for (let i = lim1; i <= lim2; i++) {
      let ii = Math.pow(i,2);
      if (a <= ii && ii <= b && allEven(ii)) res.push(ii);
  }
  return res;
  }
...
function assert (x: number[], y:number[]):void {
